...
class OperateMysql():
    ...
    def save_case_reason(self, reason_id, parent_id, name):
        ...
        sql = "insert into case_reason (case_id,parent_id,name) values('%s','%s','%s')"%(reason_id,parent_id,name)
    ...
    def query_top_level_reason(self):
        sql_1 = "select distinct(name),case_id from case_reason where parent_id='#'"
        with self.db as cursor:
    ...
    def search_childern(self,parent_id):
        ...
        sql_1 = "select distinct(name),case_id from case_reason where parent_id='%s'"% parent_id
        with self.db as cursor:
            cursor.execute(sql_1)
            name_id_tup = cursor.fetchall()
            return name_id_tup
